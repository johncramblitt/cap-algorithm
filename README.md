...
numpy, matplotlib, cartopy, xarray, rioxarray, dask
...
With pip:
```
pip install numpy matplotlib cartopy xarray rioxarray dask
```
## FILE STRUCTURE

- `data/`
    - `DEM_30m.tif` (downloaded from USGS [2024](#references))
...
# 2. run CAP analysis. 
#    - resolution = DEM grid spacing in meters (e.g., 30 for a 30m DEM).
#    - xv_dist = list of cross-valley distances in meters (e.g., [3000]).
ds = cap_analysis(terrain_data=terrain_data, resolution=30, xv_dist=[3000])
```
...
This implementation is not currently optimized for performance, and system memory limitations may arise when analyzing large regions or high-resolution DEMs. Please contact jcramblitt@berkeley.edu if you are facing significant performance issues. 
...
DEM size, resolution, and number of iterations through distance values (i.e., the length of the array passed to xv_dist) all influence algorithm speed. The algorithm is quick to run over small scales at a reasonable resolution, but is not optimized for performance and may fail under system memory constraints for large or high resolution DEMs. Future versions may include improved dask integration if this poses a barrier for users.   

### **3. Plotting**

The notebook provides a few crude plotting examples, as well as one custom plotting function to visualize CAP distributions across terrain. This function is provided for illustrative purposes only. If you use or adapt figures generated by this code for publication, please acknowledge this repository. Thank you.
...
**plotcap**(terrain_stats, xv_dist, bounding_box=None, depth_plot=False, shade=False, contour=False, linewidths=0.05, grid=True, output='show', dpi=300):
    ...
    Args:
        ...
        xv_dist: coordinate value. REQUIRED if depth_plot=False. The cross-valley distance (meters) for which to plot CAP pooling signal.
        bounding_box: list. Default = None. Lat/lon bounding box in the form [xmin, xmax, ymin, ymax]. Use WGS84.
...
**get_rank_curvature** (terrain_data, subset_elevation, margin_pixels, resolution, r):
    ...
    Calculates rank and curvature at each pixel within the digital elevation model.
